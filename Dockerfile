...
RUN DUMMY=${DUMMY}; \
    ...
    if [ "$app" = "NRC" ] ; then \
       python3 scripts/nike_sync.py ${nike_refresh_token}; \
         ...
         python3 scripts/garmin_sync.py wanglei2279@126.com Wl98622  --is-cn ; \
...
RUN python3 scripts/gen_svg.py --from-db --title "my running page" --type grid --athlete "$YOUR_NAME" --output assets/grid.svg --min-distance 12.0 --special-color yellow --special-color2 red --special-distance 20 --special-distance2 40 --use-localtime \
        && python3 scripts/gen_svg.py --from-db --title "my running page" --type github --athlete "$YOUR_NAME" --special-distance 20 --special-distance2 40 --special-color yellow --special-color2 red --output assets/github.svg --use-localtime --min-distance 0.5 \
